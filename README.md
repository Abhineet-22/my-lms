...
2. Local Development
Backend:
The backend files would handle the auth stack – register-login-verify and then proceed to route to the dashboard. This is handled by multiple Lambda functions that have specific roles setup through the IAM policies.
Once this is done and API endpoints have been configured through AWS API Gateway, you can start sending curl requests to check if the backend is up and running. Make sure to deploy the configured APIs to a stage(prod,dev etc.) in order to test them.
